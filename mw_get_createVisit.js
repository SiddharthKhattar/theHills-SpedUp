...
const fs = require('fs');
const logFilePath = 'flag_mw_visit_log.txt'; // 1
...
async function makeApiRequest() {
  ...
  let options = {
    method: 'GET',
    url: `https://sales2-dev-com-sap-rex-app.cfapps.eu30.hana.ondemand.com/com.sap.rex.app/VisitSet(${visitId})`,
    headers: {
  ...
  try {
    ...
    const logMessage = `ID: ${visitId}, Status Code: ${response.status}, Exists: Yes\n`;
    ...
    if (error.response) {
      if (error.response.status === 401) { // Unauthorized
        ...
        const logMessage = `ID: ${visitId}, Status Code: ${retryResponse.status}, Exists: Yes (After Refresh)\n`;
        Flag_create = 0;
        fs.appendFileSync('middlewarelog.txt', logMessage);
      } else {
        const logMessage = `ID: ${visitId}, Error Status Code: ${error.response.status}, Exists: No\n`;
        Flag_create = 1;
        fs.appendFileSync('middlewarelog.txt', logMessage);
      }
    } else {
      console.error("Error:", error.message);
      Flag_create = 1; // Update to indicate error
    }
  } finally {
